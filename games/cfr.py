from games.kunh.constants import ATTACKER
...
class CounterfactualRegretMinimizationBase:
    ...
    def _cfr_utility_recursive(self, state, reach_a, reach_b):
        ...
        for action in state.actions:
            child_reach_a = reach_a * (self.sigma[state.inf_set()][action] if state.to_move == ATTACKER else 1)
            child_reach_b = reach_b * (self.sigma[state.inf_set()][action] if state.to_move == -ATTACKER else 1)
        ...
        (cfr_reach, reach) = (reach_b, reach_a) if state.to_move == ATTACKER else (reach_a, reach_b)
        for action in state.actions:
