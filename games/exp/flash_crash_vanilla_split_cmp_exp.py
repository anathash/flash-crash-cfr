...
def get_split_cfr_exploitabilty(defender_budget, attacker_budgets, network, step_order_size,
    ...
    (main_game_results, selector_game_result) = split_game_cfr.run(main_game_root=root, attacker_types=attacker_budgets,
                                     ...
                                     subgame_keys=cfr_actions_mgr.get_portfolios_prob().keys())
...
def iteration_stats_assets_csv(res_dir, defender_budget, attacker_budgets, main_game_iteration_portion,
                        min_iterations, max_iterations, jump, num_assets_range, step_order_size, max_order_num, num_exp=10):
    regrets = []
    iteration_regrets = {}
    ...
    while iterations_num < max_iterations:
        ...
        for num_assets in num_assets_range:
            iteration_regrets[iterations_num].update({'split_' + str(num_assets): 0, 'vanilla_' + str(num_assets): 0})
        iterations_num += jump
    ...
    for num_assets in num_assets_range:
        ...
        while iterations_num <= max_iterations:
            for i in range(0, num_exp):
                ...
                iteration_regrets[iterations_num]['split_'+str(num_assets)] += split_cfr_exp
                iteration_regrets[iterations_num]['vanilla_'+str(num_assets)] += vanilla_cfr_exp
            iterations_num += jump
...
def iteration_stats_csv(res_dir, defender_budget, attacker_budgets, main_game_iteration_portion,
    ...
    with open(res_dir + 'exploitability_' + str(num_assets) +'.csv','w', newline='') as csvfile:
