from games.kunh.constants import CHECK, BET, CALL, FOLD, ATTACKER, CHANCE, RESULTS_MAP
import random

from games.bases import GameStateBase


class SearchRootChanceGameState(GameStateBase):

    def __init__(self, actions):
        super().__init__(parent = None, to_move = CHANCE, actions = actions)
        self.children = {
            cards: SearchAttackerMoveGameState(
                self, ATTACKER, [],  cards, [BET, CHECK]
...
class SearchAttackerMoveGameState(GameStateBase):
    ...
    def __init__(self, parent, to_move, actions_history, cards, actions):
        ...
        public_card = self.cards[0] if self.to_move == ATTACKER else self.cards[1]
